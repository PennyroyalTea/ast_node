...
## Подробнее
- Нужно считать из стандартного ввода две строки (верхний и нижний текст), используя `OpenCV`, поместить верхний текст в верхней части картинки, а нижний - в нижней части. 
- Текст должен быть белым (`#ffffff`) с черной (`#000000`) обводкой. 
- Для размещения текста на картинке рекомендуется использовать функцию `cv2.putText`. 
- Можно использовать любой шрифт, позволяющий нормально отобразить буквы латинского алфавита, кроме `Comic Sans` и `Papyrus`.
