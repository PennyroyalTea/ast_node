...
## Скрипт
Скрипт с именем `translate.py` запускается с двумя параметрами командной строки - N и API_KEY - количеством переводов в цепочке и путём к файлу с ключом API переводчика, соответственно.
...
- Картинка имеет размер хотя бы 800x450 пикселей.
- Любое сочетание из корректных строчек должно целиком помещаться на картинке.
